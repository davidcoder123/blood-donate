...
router.post("/requests", async (req, res) => {
  try {
    const { bloodGroup, unitsNeeded, urgency, patientName, notes, radiusKm } =
      req.body;
    ...
    const populatedReq = await BloodRequest.findById(request._id).populate(
      "hospital",
      "hospitalName city state phone location address",
    );

    // Collect FCM tokens for Push Notifications
    const fcmTokens = [];

    // Send email, DB notification, Socket.io, and collect FCM tokens
    ...
    // Send Firebase Push Notifications in batch (Multicast)
    if (fcmTokens.length > 0) {
      const message = {
        notification: {
          title: `🩸 Urgent ${bloodGroup} Blood Needed!`,
          body: `${req.user.hospitalName} in ${req.user.city} needs ${unitsNeeded} units of ${bloodGroup} blood.`,
        },
        data: {
      ...
      admin
        ...
        .then((response) => {
          console.log(
            `${response.successCount} Firebase push notifications sent successfully.`,
          );
        })
        .catch((error) => {
...
router.get("/requests", async (req, res) => {
  try {
    const requests = await BloodRequest.find({ hospital: req.user._id }).sort({
      createdAt: -1,
    });
    res.json(requests);
  } catch (err) {
...
router.get("/requests/:id/responses", async (req, res) => {
  try {
    const request = await BloodRequest.findById(req.params.id).populate(
      "responses.donor",
      "name bloodGroup phone city available",
    );
...
router.patch(
  "/requests/:requestId/confirm-donation/:donorId",
  async (req, res) => {
    try {
      ...
      if (!response)
        return res.status(404).json({ error: "Donor response not found." });
      if (response.status === "donated")
      ...
      const updatedDonor = await User.findByIdAndUpdate(req.params.donorId, {
        lastDonated: today,
        $inc: { donationCount: 1 },
      ...
      // Send push notification for donation confirmation if token exists
      if (updatedDonor && updatedDonor.fcmToken) {
        admin
          .messaging()
          .send({
            token: updatedDonor.fcmToken,
            notification: {
              title: "Donation Confirmed! 🎉",
              body: `${req.user.hospitalName} confirmed your donation. Thank you for saving a life!`,
            },
          })
          .catch(console.error);
      }
      ...
      res.json({
        message: "Donation confirmed! Donor blocked for 90 days.",
        eligibleAfter: blockUntil,
      });
    } catch (err) {
      console.error(err);
