...
router.post('/send-otp', otpLimiter, [
  ...
  try {
    ...
    await User.findOneAndUpdate(
      ...
      { email, otp, otpExpiry, role: 'donor', status: 'pending', name: 'temp' },
...
router.post('/register/donor', [
  ...
  try {
    const { name, email, password, otp, bloodGroup, phone, address, city, state, latitude, longitude } = req.body;
    // Find temp user with OTP
    const tempUser = await User.findOne({ email }).select('+otp +otpExpiry');
    if (!tempUser || !tempUser.otp) return res.status(400).json({ error: 'No OTP found. Please request a new OTP.' });
    if (tempUser.otpExpiry < new Date()) return res.status(400).json({ error: 'OTP expired. Please request a new one.' });
    if (tempUser.otp !== otp) return res.status(400).json({ error: 'Incorrect OTP. Please try again.' });
    ...
    if (latitude && longitude) {
      tempUser.location = { type: 'Point', coordinates: [parseFloat(longitude), parseFloat(latitude)] };
    }
    await tempUser.save();
...
router.post('/register/hospital', [
  ...
  try {
    const { name, email, password, hospitalName, licenseNumber, phone, address, city, state, latitude, longitude } = req.body;
    ...
    res.status(201).json({ message: 'Registration submitted. You will receive an email once approved by admin.' });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Registration failed. Please try again.' });
  }
...
router.post('/login', loginLimiter, [
  ...
  try {
    ...
    if (!user) return res.status(401).json({ error: 'Invalid email or password.' }); // Same message for security

    const isMatch = await user.comparePassword(password);
    if (!isMatch) return res.status(401).json({ error: 'Invalid email or password.' });

    if (user.status === 'pending') return res.status(403).json({ error: 'Account pending admin approval. Check your email.' });
    if (user.status === 'rejected') return res.status(403).json({ error: 'Account was rejected. Contact support.' });
