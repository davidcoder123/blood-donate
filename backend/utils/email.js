...
// Helper function to send via EmailJS HTTPS API
const sendEmail = async ({ to_email, subject, html }) => {
  try {
    const response = await emailjs.send(
      ...
      process.env.EMAILJS_TEMPLATE_ID,
      {
        to_email: to_email,
        subject: subject,
        message_html: html, // Sends raw HTML to EmailJS
      },
      {
    ...
    console.log(`✅ Email sent to ${to_email}:`, response.status);
    return response;
  } catch (error) {
    console.error(`❌ EmailJS Error sending to ${to_email}:`, error);
    throw error;
  }
};
