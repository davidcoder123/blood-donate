...
export default function RegisterDonor() {
    ...
    try {
      for (const q of queries) {
        ...
        if (data.length) {
          ...
          toast.success(
            `Location set! (matched: ${data[0].display_name.substring(0, 60)}...)`,
          );
          return;
        }
      }
  ...
  const s = {
    ...
    dot: (a) => ({
      ...
      height: "30px",
      borderRadius: "50%",
      background: a ? "#2563eb" : "#e5e7eb",
      color: a ? "white" : "#9ca3af",
      display: "flex",
      alignItems: "center",
      justifyContent: "center",
      fontSize: "13px",
      ...
      <div style={s.card}>
        ...
        {step === 2 && (
          <div>
            ...
            />
            <button style={s.btn} onClick={handleVerifyOtp}>
              Verify OTP →
            </button>
            <button
              style={{ ...s.outBtn, marginTop: "10px" }}
              onClick={handleSendOtp}
              disabled={loading}
            >
              Resend OTP
        ...
        {step === 3 && (
          <form onSubmit={handleSubmit}>
            ...
            {form.latitude && (
              <div style={s.locBox}>
                ✅ Location captured: {parseFloat(form.latitude).toFixed(4)},{" "}
