...
export const respondToRequest = (id, accept) =>
  API.post(`/donor/requests/${id}/respond`, { accept });
export const getDonorHistory = () => API.get("/donor/history");
export const toggleAvailability = (available) =>
  API.patch("/donor/availability", { available });
export const confirmDonation = (requestId, donorId) =>
  API.patch(`/hospital/requests/${requestId}/confirm-donation/${donorId}`);
...
export const getResponses = (id) =>
  API.get(`/hospital/requests/${id}/responses`);
export const updateReqStatus = (id, status) =>
  API.patch(`/hospital/requests/${id}/status`, { status });
...
export const approveHospital = (id, status) =>
